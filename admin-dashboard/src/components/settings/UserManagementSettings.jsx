...
const UserManagementSettings = ({ onSave }) => {
    ...
    const [settings, setSettings] = useState({
        ...
        revenuePerPickup: globalSettings.revenuePerPickup || 5.50
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        updateSettings({
            revenuePerPickup: parseFloat(settings.revenuePerPickup)
        });
        onSave(settings);
        setIsDirty(false);
    };
        ...
        <Card title={t('User Management Settings')}>
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-4">
                    ...
                    <Input
                        label={t('Revenue per Pickup ($)')}
                        description={t('Amount earned per successful package pickup')}
                        type="number"
                        step="0.01"
                        name="revenuePerPickup"
                        value={settings.revenuePerPickup}
                        onChange={handleChange}
                        min="0"
                    />
                </div>
