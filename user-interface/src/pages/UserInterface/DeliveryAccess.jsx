...
import { ArrowLeft, Hash, ArrowRight, Package, AlertTriangle, CheckCircle } from "lucide-react";
...
export default function DeliveryAccess() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/apartment/delivery", {
        method: "POST",
      ...
      if (!response.ok || !data.success) {
        setError(data.message || "Apartment ID not found.");
        return;
      }

      setSuccess(`Package for ${data.nameOfOwner} placed in Locker ${data.lockerId}. OTP sent to resident's email.`);
      setApartmentId("");
    } catch (error) {
        ...
        <div className="bg-slate-900/40 backdrop-blur-2xl rounded-[2.5rem] border border-white/10 p-10 relative shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="group w-full py-4 rounded-2xl bg-indigo-600 text-white font-semibold text-lg hover:bg-indigo-500 transition-all duration-300 flex items-center justify-center gap-2 shadow-lg shadow-indigo-900/20 hover:shadow-indigo-500/40 hover:-translate-y-1 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:translate-y-0"
            >
              {loading ? (
